global triciclo alfa vAngular vLineal phi P theta tiempo ruta contador pintaRuta obs1 obs2  navegar;
...
for i=1:tiempo
    ...
    if PuntoM(1)~=0 && PuntoM(2)~=0
        plot(PuntoM(1),PuntoM(2),'greenX','LineWidth',1), hold on
    end
...
elseif simuDiferencial==1  
    ...
    CCI=[(P(1)+R*cosd(theta)) , (P(2)-R*sind(theta)) ];
     ...
     elseif (vLinealL==-vLinealR)
         vAngularD=((2/d1)*(vLinealR))*180/pi;
        for jj=1:3
             punto=[triciclo(1,jj)-CCI(1);
                    triciclo(2,jj)-CCI(2);
                    0];
             ICCmas=[CCI(1);
                     CCI(2);
                     0];
             mRot=[cosd(vAngularD)   sind(vAngularD)   0;
                   -sind(vAngularD)  cosd(vAngularD)   0;
             	    0          0           1]; 
             nx=mRot*punto+ICCmas;
             triciclo(1,jj)=nx(1);
             triciclo(2,jj)=nx(2);
	...
	set(handles.text5,'string', P(1))
	set(handles.text6,'string', P(2))
    set(handles.text8,'string', vAngularL)
	set(handles.text9,'string', vLinealL)
	set(handles.text11,'string','--') % alfa del triciclo
    set(handles.text20,'string',vAngularR)
    set(handles.text21,'string',vLinealR)
    set(handles.text13,'string',theta)
	set(handles.text15,'string',CCI1P)
	set(handles.text16,'string',CCI1P)
end
...
%     fill(obs3(1,:),obs3(2,:),'black'); hold on
    ...
    xlabel('x'); ylabel('y');
	title(titulo);axis([-110 110 -110 110]),grid on
    pause(0.2)
end